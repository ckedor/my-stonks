...
def calculate_risk_metrics(returns, cdi_returns):
    ...
    dd_serialized = dd_df['drawdown'].reset_index().rename(columns={'index': 'date'})
...
def calculate_performance_metrics(returns, benchmarks):
    ...
    for name, benchmark_history in benchmarks.items():
        ...
        beta = (
            corr * (returns.std() / benchmark_returns.std()) if benchmark_returns.std() > 0 else 0
        )
    ...
    return {'cagr': portfolio_cagr * 100, 'benchmarks_metrics': benchmarks_metrics}
